...
export default function AdminAIUsagePage() {
  ...
  useEffect(() => {
    (async () => {
      const data = await apiClient.adminAIUsage(params);
      setRows(data || []);
    })();
  }, [params]);
  ...
  return (
    <div className="flex flex-wrap gap-3 items-end">
      ...
      <div className="flex-1 min-w-[220px]">
        ...
        <input placeholder="/ai" value={route} onChange={(e) => onRoute(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded px-2 py-1" />
      </div>
    </div>
  );
}
